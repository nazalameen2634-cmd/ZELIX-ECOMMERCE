...
export async function POST(request: Request) {
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      try {
        await resend.emails.send({
          ...
          to: email,
          subject: 'ZELIX Verification Code',
          html: `
            <div style="font-family: 'Inter', sans-serif; max-width: 400px; margin: 0 auto; padding: 40px 20px; text-align: center; background-color: #000; color: #fff; border-radius: 12px;">
              ...
              <p style="font-size: 16px; color: #a1a1aa; margin-bottom: 20px;">Use the following verification code to create your account.</p>
    ...
    return NextResponse.json({ success: true, message: 'OTP has been sent.' });
  } catch (error) {
